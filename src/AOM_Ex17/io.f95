...
!=======================
    SUBROUTINE create_nc(outfile)
        ...
        CALL check(nf90_def_var(ncid, "v", NF90_FLOAT, dimids, v_id))
        ...
        CALL check(nf90_def_var(ncid, "c1", NF90_FLOAT, dimids, c1_id))
        CALL check(nf90_def_var(ncid, "c2", NF90_FLOAT, dimids, c2_id))
        CALL check(nf90_def_var(ncid, "c3", NF90_FLOAT, dimids, c3_id))
...
!:=========================================================================
    SUBROUTINE write_nc(outfile, time)
        ...
        INTEGER :: p_id, q_id, u_id, v_id, w_id, time_id, rho_id
        INTEGER :: c1_id, c2_id, c3_id 
        INTEGER :: n
        ...
        CALL check(nf90_put_var(ncid, v_id, v(1:nz, 1:nx), start=(/1, 1, n + 1/), count=(/nz, nx, 1/)))
        ...
        CALL check(nf90_inq_varid(ncid, "c1", c1_id))
        CALL check(nf90_put_var(ncid, c1_id, c1(1:nz, 1:nx), start=(/1, 1, n + 1/), count=(/nz, nx, 1/)))

        CALL check(nf90_inq_varid(ncid, "c2", c2_id))
        CALL check(nf90_put_var(ncid, c2_id, c2(1:nz, 1:nx), start=(/1, 1, n + 1/), count=(/nz, nx, 1/)))

        CALL check(nf90_inq_varid(ncid, "c3", c3_id))
        CALL check(nf90_put_var(ncid, c3_id, c3(1:nz, 1:nx), start=(/1, 1, n + 1/), count=(/nz, nx, 1/)))
