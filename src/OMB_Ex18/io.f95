...
!=======================
    SUBROUTINE create_nc(outfile)
        ...
        CALL check(nf90_def_var(ncid, "h", NF90_FLOAT, (/yh_dimid, xh_dimid, time_dimid/), h_id))
        CALL check(nf90_def_var(ncid, "eta", NF90_FLOAT, (/yh_dimid, xh_dimid, time_dimid/), eta_id))
        CALL check(nf90_def_var(ncid, "u", NF90_FLOAT, (/yu_dimid, xu_dimid, time_dimid/), u_id))
        CALL check(nf90_def_var(ncid, "v", NF90_FLOAT, (/yv_dimid, xv_dimid, time_dimid/), v_id))
        CALL check(nf90_def_var(ncid, "T", NF90_FLOAT, (/yh_dimid, xh_dimid, time_dimid/), T_id))
...
!:=========================================================================
    SUBROUTINE write_nc(outfile, time)
        ...
        CALL check(nf90_put_var(ncid, h_id, h, start=(/1, 1, n + 1/), count=(/ny+2, nx+2, 1/)))

        CALL check(nf90_inq_varid(ncid, "u", u_id))
        CALL check(nf90_put_var(ncid, u_id, u, start=(/1, 1, n + 1/), count=(/ny+2, nx+2, 1/)))

        CALL check(nf90_inq_varid(ncid, "v", v_id))
        CALL check(nf90_put_var(ncid, v_id, v, start=(/1, 1, n + 1/), count=(/ny+2, nx+2, 1/)))

        CALL check(nf90_inq_varid(ncid, "eta", eta_id))
        CALL check(nf90_put_var(ncid, eta_id, eta, start=(/1, 1, n + 1/), count=(/ny+2, nx+2, 1/)))

        CALL check(nf90_inq_varid(ncid, "T", T_id))
        CALL check(nf90_put_var(ncid, T_id, T, start=(/1, 1, n + 1/), count=(/ny+2, nx+2, 1/)))
